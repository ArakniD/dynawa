...
{
  if(Io_isrSourceCount >= MAX_INTERRUPT_SOURCES)
    return false;
  
  Io_isrSources[Io_isrSourceCount].port = io->basePort;
  Io_isrSources[Io_isrSourceCount].mask = io->mask;
  ...
  io->basePort->PIO_IER = Io_isrSources[Io_isrSourceCount].mask; // enable our channel

  Io_isrSources[Io_isrSourceCount].handler = h;
  Io_isrSources[Io_isrSourceCount++].context = context; // make sure to increment our handler count
